    ...
    for name, module in model.model.named_modules():
        if isinstance(module, nn.BatchNorm2d):
            ...
            if name not in ignore_bn_list:
                ...
                # ======================================================================
                # THÊM CODE: Ép số lượng mask channel phải chia hết cho 8 (TensorRT Optimize)
                # ======================================================================
                current_channels = mask.sum().int().item()
                target_channels = make_divisible(current_channels, 8)
                target_channels = min(target_channels, origin_channels)  # Đảm bảo không vượt quá số kênh gốc

                if current_channels != target_channels and target_channels > 0:
                    gamma_abs = module.weight.data.abs().clone().view(-1)
                    kth_value = torch.topk(gamma_abs, target_channels)[0][-1]
                    mask = gamma_abs.ge(kth_value).float()
                # ======================================================================

...
def coco_eval(gt_json, pred_json, stem2id):
    ...
    return [round(float(v) * 100, 2) for v in e.stats[:6]]
...
def main():
    ...
    for c in ckpts:
        ...
        if not pred.exists():
            ...
            continue
        stats = coco_eval(gt_json, pred, stem2id)
        par = sum(p.numel() for p in m.model.parameters()) / 1e6
        rows.append((c.stem, round(par, 2), *stats))

    print("\n" + "=" * 92)
    print("{:<30} {:>8} {:>7} {:>7} {:>7} {:>7} {:>7} {:>7}".format(
        "model", "par M", "AP", "AP50", "AP75", "APs", "APm", "APl"))
    print("-" * 92)
    for r in rows:
        print("{:<30} {:>8.2f} {:>7} {:>7} {:>7} {:>7} {:>7} {:>7}".format(*r))

    out = ROOT / "results" / "visdrone" / "logs" / "ap_by_size.json"
    out.write_text(json.dumps(
        [dict(zip(("model", "params_M", "AP", "AP50", "AP75", "APs", "APm", "APl"), r))

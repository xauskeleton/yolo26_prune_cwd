...
def parse_args():
    ...
    parser.add_argument('--weights', type=str, default='weights/yolo26m_baseline.pt',
                        help='baseline model path')
    parser.add_argument('--cfg', type=str, default='cfg/yolo26m.yaml',
                        help='model YAML config')
    parser.add_argument('--data', type=str, default='VOC.yaml',
                        help='dataset config for finetune')
    parser.add_argument('--model-size', type=str, default='m',
                        choices=['n', 's', 'm', 'l', 'x'])
    parser.add_argument('--iters', type=int, default=5,
                        help='number of prune iterations')
    parser.add_argument('--prune-ratio', type=float, default=0.13,
                        help='prune ratio per iteration (0.0-1.0)')
    parser.add_argument('--epochs', type=int, default=10,
                        help='finetune epochs per iteration (0 = skip finetune)')
    parser.add_argument('--batch', type=int, default=16,
                        help='finetune batch size')
    parser.add_argument('--imgsz', type=int, default=640,
                        help='finetune image size')
    parser.add_argument('--device', type=str, default='0',
                        help='device for finetune')
    parser.add_argument('--divisor', type=int, default=8,
                        choices=[8, 16])
    parser.add_argument('--save-dir', type=str, default='weights/iterative',
                        help='output directory')
    parser.add_argument('--kd', action='store_true',
                        help='use CWD distillation during finetune')
    parser.add_argument('--kd-teacher', type=str, default=None,
                        help='teacher model path (default: same as --weights)')
...
if __name__ == "__main__":
    ...
    for i in range(1, opt.iters + 1):
        ...
        model, bn_dict, ignore_bn_list, _, layer_ratio_cfg, pruned_yaml = \
            load_and_prepare(current, opt.cfg, opt.model_size, None)
        ...
        pruned_path = finalize_pruning(
            ...
            current, str(save_dir), opt.divisor, opt.prune_ratio,

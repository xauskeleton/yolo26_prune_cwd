...
class DetectPruned(nn.Module):
    ...
    def __init__(
        ...
        if reg_max != 1:
            import warnings
            warnings.warn(
                f"YOLO26 uses reg_max=1 (no DFL), but got reg_max={reg_max}. "
        ...
        for i in range(self.nl):
            self.cv3.append(
                nn.Sequential(
                    # Layer 0: DWConv → Conv
                    nn.Sequential(
                        DWConv(ch[i], cv3x0_dw_outs[i], 3),
                        Conv(cv3x0_dw_outs[i], cv3x0_pw_outs[i], 1)
                    ),
                    # Layer 1: DWConv → Conv
                    nn.Sequential(
                        DWConv(cv3x0_pw_outs[i], cv3x1_dw_outs[i], 3),
                        Conv(cv3x1_dw_outs[i], cv3x1_pw_outs[i], 1)
                    ),
                    # Final: Conv2d
